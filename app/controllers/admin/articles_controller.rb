class Admin::ArticlesController < ApplicationController
  ...
  def update
    ...
    @article.assign_article_state unless @article.draft?
    if @article.save!
      flash[:notice] = '更新しました'
      redirect_to edit_admin_article_path(@article.uuid)
    else
  ...
  def article_params
    params.require(:article).permit(
      :title, :description, :slug, :state, :published_at, :eye_catch, :category_id, :author_id, :eyecatch_width, :eyecatch_position, tag_ids: []
    )
  end
